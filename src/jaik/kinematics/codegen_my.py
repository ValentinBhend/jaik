...
def _sp1(p1, p2, k):
    """SP1: all sympy. Returns scalar theta."""
    KxP = k.cross(p1)
    x0  = KxP.dot(p2)
...
def _sp3(p1, p2, k, d):
    """SP3: all sympy. Returns (theta1, theta2) — NaN via sqrt when unreachable."""
    KxP       = k.cross(p1)
    A1 = sp.Matrix([[*KxP], [*k.cross(KxP)]])    # (2, 3)
    A        = -2 * A1 * p2           # (2,)
    norm_A_sq = (A.T * A)[0]
    norm_A    = sp.sqrt(norm_A_sq)
    u = p2 - (k * p1.T) * k
    b         = (d**2 - (u.T * u)[0] - sp.sqrt((KxP.T*KxP)[0]))
    x_ls  = A1 * (-2 * p2 * b / norm_A_sq)
    xi    = sp.sqrt(1 - b**2 / norm_A_sq)              # NaN if unreachable
    A_perp = sp.Matrix([A[1,:], -A[0,:]]) / norm_A
    sc1 = sp.Matrix([x_ls + xi * A_perp])
    sc2 = sp.Matrix([x_ls - xi * A_perp])
    return sp.atan2(sc1[0], sc1[1]), sp.atan2(sc2[0], sc2[1])


def _sp4(p, k, h, d):
    """SP4: all sympy. Returns (theta1, theta2) — NaN via sqrt when unreachable."""
    A11          = k.cross(p)
    A1           = sp.Matrix([[*A11], [*(-k.cross(A11))]])  # (2, 3)
    A            = h * A1.T                                # (2,)
    b            = d - h.dot(k) * k.dot(p)
    norm_A2      = A * A
    x_ls         = A1 * (h * b)
    xi           = sp.sqrt(norm_A2 - b**2)               # NaN if unreachable
...
def _derive_staged(H_num, P_num):
    ...
    """
    # H_syms = [[int(round(H_num[i,j])) if _is_int(H_num[i,j]) else sp.Symbol(f'H{i+1}{j+1}') for j in range(6)] for i in range(3)]
    # P_syms = [[int(round(P_num[i,j])) if _is_int(P_num[i,j]) else sp.Symbol(f'P{i+1}{j+1}') for j in range(7)] for i in range(3)]
    H_syms = [[H_num[i,j] for j in range(6)] for i in range(3)]
    P_syms = [[P_num[i,j] for j in range(7)] for i in range(3)]
    H = sp.Matrix(H_syms)
    P = sp.Matrix(P_syms)
